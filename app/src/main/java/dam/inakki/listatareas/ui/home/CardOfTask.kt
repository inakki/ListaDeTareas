    ...
    Row(
        ...
        IconButton(
            onClick = {
                coroutineScope.launch{
                    tasks.updateTask(task.copy(state = !task.state))
                }
            }
        ) { IconDone(isDone = task.state) }
        ...
        Text(
            ...
            style = MaterialTheme.typography.bodyLarge.copy(
                textDecoration = if(task.state) TextDecoration.LineThrough else TextDecoration.None,
                color = if(task.state) MaterialTheme.colorScheme.TextTaskDone else MaterialTheme.colorScheme.TextTaskNotDone
            )
        ) // Text
